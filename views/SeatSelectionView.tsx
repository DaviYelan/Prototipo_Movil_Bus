  ...
  return (
    <div className="flex flex-col h-full bg-background-light dark:bg-background-dark text-primary dark:text-white">
      <nav className="sticky top-0 z-50 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md px-4 py-4 pt-12 flex items-center justify-between border-b border-neutral-200 dark:border-neutral-800">
        ...
        <button className="w-10 h-10 flex items-center justify-center rounded-full border border-neutral-200 dark:border-neutral-800">
      ...
      <div className="px-4 py-6">
        <div className="flex justify-between items-center bg-white dark:bg-surface-dark p-4 rounded-xl shadow-sm border border-neutral-100 dark:border-neutral-800">
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-sm bg-white border border-neutral-300 dark:border-neutral-600"></div>
            <span className="text-xs font-semibold text-neutral-500">Available</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-sm bg-primary dark:bg-neutral-800"></div>
            <span className="text-xs font-semibold text-neutral-500">Occupied</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-sm bg-accent-emerald"></div>
        ...
        <div className="max-w-md mx-auto">
          ...
          <div className="grid grid-cols-5 gap-y-6">
            {['01', '02', 'gap', '03', '04', '05', '06', 'gap', '07', '08', '09', '10', 'gap', '11', '12', '13', '14', 'gap', '15', '16', '17', '18', 'gap', '19', '20', '21', '22', 'gap', '23', '24'].map((id, index) => (
              id === 'gap' ? <div key={`gap-${index}`} className="col-span-1 flex items-center justify-center"><span className="material-symbols-outlined text-neutral-200 dark:text-neutral-800 rotate-90 scale-75">straight</span></div> : renderSeat(id)
            ))}
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 pb-12 bg-white dark:bg-background-dark border-t border-neutral-100 dark:border-neutral-800 z-[60]">
        <div className="max-w-md mx-auto flex items-center justify-between gap-4">
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              <span className="text-accent-emerald text-lg font-bold">{selectedSeats.length}</span>
              <span className="text-primary dark:text-white font-bold text-base">Seats Selected</span>
            </div>
            <p className="text-neutral-500 text-sm font-medium">Total: <span className="text-primary dark:text-white">${(selectedSeats.length * (trip?.price || 0)).toFixed(2)}</span></p>
          </div>
          <button 
            disabled={selectedSeats.length === 0}
            onClick={() => onNavigate(View.PAYMENT)}
            className={`flex-1 h-14 rounded-xl font-bold flex items-center justify-center gap-2 shadow-xl active:scale-95 transition-all ${selectedSeats.length > 0 ? 'bg-primary dark:bg-accent-emerald text-white dark:text-primary shadow-emerald-500/20' : 'bg-neutral-200 dark:bg-neutral-800 text-neutral-400 cursor-not-allowed'}`}
          >
